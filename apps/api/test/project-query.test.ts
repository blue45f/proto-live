...
async function withSeededService(
  seed: SeedFn,
  run: (service: ProjectsService) => Promise<void> | void
) {
    ...
    (service) => {
      assert.throws(
        ...
        (error: unknown) => {
          ...
          assert.equal(
            (error as ForbiddenException).message,
      ...
      const investorLogin = service.login({
        email: 'investor@protolive.local',
        password: 'pass-investor',
      })
...
test('session secret resolver requires explicit secret in production', async () => {
  const module =
    (await import('../src/projects/projects.service')) as typeof import('../src/projects/projects.service') & {
      resolveSessionSecret?: (env: Partial<NodeJS.ProcessEnv>) => string
    }

  assert.equal(typeof module.resolveSessionSecret, 'function')
  assert.equal(
    module.resolveSessionSecret!({ PROTOLIVE_SESSION_SECRET: ' configured-secret ' }),
    'configured-secret'
  )
...
test('getAllProjects supports category/search/access mode query filters', async () => {
  await withSeededService(
    ...
    async (service) => {
      ...
      assert.equal(
        aiProjects.every((project) => project.category === 'AI & SaaS'),
        true
      )
      ...
      assert.deepEqual(
        verifiedIds.slice().sort((a, b) => a - b),
        [1, 2, 4]
      )
      ...
      assert.deepEqual(
        tagSearchHits.map((project) => project.id),
        [1]
      )
      ...
      assert.deepEqual(
        highSignal.map((project) => project.id).sort((a, b) => a - b),
        [1, 4]
      )
...
test('getProjectList paginates projects and applies funding filters', async () => {
  await withSeededService(
      ...
      state.projects.push(
        {
          ...
          validation: {
            ...
            message: 'ok',
            checkedAt: '2026-06-01T00:00:00.000Z',
            finalUrl: 'https://small.example.com',
            responseTimeMs: 40,
          },
          createdAt: new Date('2026-06-01T12:00:00.000Z'),
        },
        {
          ...
          validation: {
            ...
            message: 'ok',
            checkedAt: '2026-06-01T00:00:00.000Z',
            finalUrl: 'https://middle.example.com',
            responseTimeMs: 60,
          },
          createdAt: new Date('2026-06-01T11:00:00.000Z'),
        },
    ...
    async (service) => {
      ...
      assert.deepEqual(
        firstPage.data.map((project) => project.id),
        [3, 2]
      )
      ...
      const requestedOverflowPage = await service.getProjectList({
        page: 9,
        limit: 2,
        sort: 'funding',
      })
      ...
      const fundedByRange = await service.getProjectList({
        sort: 'funding',
        minFundingAmount: 200,
        maxFundingAmount: 900,
      })
      assert.equal(fundedByRange.total, 2)
      assert.deepEqual(
        fundedByRange.data.map((project) => project.id),
        [3, 2]
      )
      ...
      assert.deepEqual(
        atMostMax.data.map((project) => project.id),
        [2, 1]
      )
    }
  )
    ...
    (state) => {
      ...
      state.projects.push({
        ...
        validation: {
          ...
          message: 'ok',
          checkedAt: '2026-06-01T00:00:00.000Z',
          finalUrl: 'https://invalid-funding.example.com',
          responseTimeMs: 50,
        },
        createdAt: new Date('2026-06-01T12:00:00.000Z'),
      })
...
test('getAdminDashboard returns health, risks, and action recommendations', async () => {
  await withSeededService(
    ...
    async (service) => {
      ...
      assert.equal(
        dashboard.riskProjects.some((project) => project.title.includes('Silent Risky Project')),
        true
      )
...
test('getAdminRevenueProjection applies formulas and overrides assumptions', async () => {
  await withSeededService(
    ...
    async (service) => {
      ...
      assert.equal(
        defaultProjection.monthlyMakerPlanRevenue > overrideProjection.monthlyMakerPlanRevenue,
        true
      )
    }
  )
...
test('getAdminDashboard includes revenue projection payload', async () => {
  await withSeededService(
    ...
    async (service) => {
      ...
      assert.equal(
        dashboardProjection.targetGap.drivers.every((driver) =>
          Number.isFinite(driver.acquisitionCostPerUnit)
        ),
        true
      )
      assert.equal(
        dashboardProjection.targetGap.drivers.every((driver) =>
          Number.isFinite(driver.estimatedPaybackMonths)
        ),
        true
      )
      assert.equal(
        dashboardProjection.benchmarkGaps.every((entry) =>
          ['good', 'warning', 'critical'].includes(entry.status)
        ),
        true
      )
