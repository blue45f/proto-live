 ...
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱(proto-live·offhours·resume·…)에 그대로 복붙해서 쓰는 버전입니다.
 * 워크스페이스 의존(@notifydesk/shared)·사이드카 파일(client/icons/styles/relative-time) 0 —
 * 필요한 타입·클라이언트·아이콘·스타일·상대시간 유틸을 이 파일에 인라인했습니다.
 * 동작/디자인은 @notifydesk/widget 의 <NotificationBell> 과 동일합니다.
 *
 * 출처: notifydesk/packages/widget/src/{react,client,icons,relative-time,styles}.tsx (벤더 원본).
 * 업스트림 갱신 시 이 파일을 교체하세요.
 *
 * 이 앱은 이미 자체 헤더 알림 벨(../../NotificationBell)을 갖고 있어 이름 충돌을 피하려
 * 컴포넌트명을 DeskNotificationBell 로 노출합니다(원본 export 명은 NotificationBell).
 *
 * 사용:
 *   import { DeskNotificationBell } from './components/deskcloud/notify/DeskNotificationBell'
 *   <DeskNotificationBell recipientId="user_42" publishableKey="pk_demo" endpoint={import.meta.env.VITE_NOTIFYDESK_URL} />
 *
 ...
 *
 * 접근성/디자인: focus-visible · 포커스 트랩 · Esc · prefers-reduced-motion ·
 * 대비 ≥4.5:1 · 그라디언트 텍스트/글래스모피즘 없음 · 외부 CSS 프레임워크 0(스코프 .nd-*).
 * ──────────────────────────────────────────────────────────────────────────
 */
import {
  useCallback,
...
export interface DeskNotificationBellProps {
...
export function DeskNotificationBell(props: DeskNotificationBellProps): ReactElement {
  const {
    recipientId,
    publishableKey,
    endpoint,
...
export default DeskNotificationBell
export type { NotificationDto, NotifyDeskWidgetClient }
