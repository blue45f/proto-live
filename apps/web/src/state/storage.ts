...
export function readAdminRevenueConfig(): RevenueModelConfig {
  ...
  try {
    ...
    return {
      makerMonthlyFee: safePositiveNumber(
        parsed.makerMonthlyFee,
        DEFAULT_REVENUE_CONFIG.makerMonthlyFee
      ),
      investorMonthlyFee: safePositiveNumber(
        parsed.investorMonthlyFee,
        DEFAULT_REVENUE_CONFIG.investorMonthlyFee
      ),
      leadCaptureFee: safePositiveNumber(
        parsed.leadCaptureFee,
        DEFAULT_REVENUE_CONFIG.leadCaptureFee
      ),
      ...
      estimatedMonthlyChurnRate: clampRate(
        safeNumber(
          parsed.estimatedMonthlyChurnRate,
          DEFAULT_REVENUE_CONFIG.estimatedMonthlyChurnRate
        ),
        0.01,
        99.99
      ),
      makerConversionRate: clampRate(
        safeNumber(parsed.makerConversionRate, DEFAULT_REVENUE_CONFIG.makerConversionRate)
      ),
      ...
      closeLeadRate: clampRate(
        safeNumber(parsed.closeLeadRate, DEFAULT_REVENUE_CONFIG.closeLeadRate)
      ),
      successFeeRate: clampRate(
        safeNumber(parsed.successFeeRate, DEFAULT_REVENUE_CONFIG.successFeeRate),
        0.1,
        30
      ),
    }
...
export function readAdminRevenueTarget(): number {
  ...
  try {
    ...
    return safePositiveNumber(
      (parsed as { targetMonthlyRevenue?: unknown }).targetMonthlyRevenue,
      DEFAULT_REVENUE_TARGET
    )
  } catch {
    return DEFAULT_REVENUE_TARGET
...
export function readFilterPreset(): RawFilterSnapshot {
  ...
  if (!hasAnyQuery) {
    return {
      ...
      minSignal: Math.max(
        0,
        safeInt(stored.minSignal?.toString() ?? null, fallback.minSignal ?? 0)
      ),
