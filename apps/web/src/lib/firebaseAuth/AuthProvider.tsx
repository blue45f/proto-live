...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const run = useCallback(async (op: () => Promise<unknown>): Promise<void> => {
    ...
    try {
      ...
      // 정규화한 메시지로 다시 throw 하되 원인(cause)을 보존한다.
      // (이 앱 tsconfig 의 lib 은 ES2020 이라 Error options 인자/타입드 cause 를 쓰지 않고
      // 런타임 속성으로만 부착 — 디버깅용이며 사용자 노출 계약은 state.error 메시지다.)
      const normalized = new Error(message)
      Object.assign(normalized, { cause: err })
      throw normalized
    }
  }, [])
