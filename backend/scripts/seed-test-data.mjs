    ...
    .map((entry) => {
      ...
      return {
        ...
        validation: {
          ...
          ...(safeString(rawValidation.finalUrl) ? { finalUrl: safeString(rawValidation.finalUrl) } : {}),
        },
...
function seed(state, fixtureState) {
  ...
  const usedUserIds = new Set(state.users.map((user) => safeParseInt(user.id, 0)).filter(Number.isInteger));
  ...
  const userIds = new Set(state.users.map((user) => safeParseInt(user.id, 0)).filter((id) => Number.isInteger(id)));
  ...
  const validProjectIds = new Set(state.projects.map((project) => safeParseInt(project.id, 0)).filter((id) => Number.isInteger(id)));

  const proposalCandidates = fixtureState.proposals.filter((proposal) => validProjectIds.has(proposal.projectId));
  ...
  const eventCandidates = fixtureState.events.filter((event) => validProjectIds.has(event.projectId));
