...
function seedAccounts(state, fixtureAccounts) {
  ...
  const maxExistingId = state.users.reduce((acc, user) => Math.max(acc, safeParseInt(user.id, 0)), 0);
...
function main() {
  ...
  console.log(`테스트 계정 시드 완료: ${uniqueSeeded.length}개 계정 동기화, 사용자 수 ${beforeCount} → ${afterCount}`);
